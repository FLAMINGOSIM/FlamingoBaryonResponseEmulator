...
FLAMINGO_labels = [
    ...
    "${\\rm fgas}-8\\sigma$",
    "${\\rm M*}-\\sigma$",
    "${\\rm M*}-\\sigma+{\\rm fgas}-4\\sigma$",
    "${\\rm Jets}$",
    "${\\rm Jets}~{\\rm fgas}-4\\sigma$",
    "Planck",
    "PlanckNu0p24Fix ",
    "PlanckNu0p24Var",
    "LS8",
...
def PS_ratio(
    ...
    if fix_low_k_norm and jet == 1.0:
        offset = 0.99985 + 0.000034 * z - interpolator(-2.0)
        P_data += offset
        interpolator = inter.CubicSpline(np.log10(k_data), P_data)
...
def make_training_data(
    ...
    colors_z = cm.plasma(np.linspace(0.0, 0.9, len(z_train)))
    ...
    for i in range(len(model_train)):
        for j in range(len(z_train)):
            ...
            my_k = 10 ** (
                ...
                + np.random.random(num_bins_k) * rand_k_norm
                - rand_k_norm / 2.0
            )
            my_R = PS_ratio(
