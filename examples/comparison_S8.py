...
(l,) = ax.plot(
    bins_k_Arico,
    (Arico_high_plot + Arico_low_plot) / 2.0,
    "-",
    color="c",
    lw=0.7,
    label="${\\rm Arico+23~(DES~WL)}$",
)
lines.append(l)
...
(l,) = ax.plot(
    ...
    color="m",
    lw=0.7,
    label="${\\rm Preston+23~(DES~WL+CMB)}$",
)
lines.append(l)
...
BigwoodkSZ_low_plot[BigwoodkSZ_low_plot > 1] = 1
ax.fill_between(
    bins_k_Arico, BigwoodkSZ_low_plot, BigwoodkSZ_high_plot, color="g", alpha=0.4
)
(l,) = ax.plot(
    BigwoodkSZ_mean_k,
    BigwoodkSZ_mean_R,
    ...
    color="g",
    lw=0.7,
    label="${\\rm Bigwood+24~(DES~WL+ACT~kSZ)}$",
)
lines.append(l)
